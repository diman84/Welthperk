...
Promise.all([window.__data ? true : isOnline(), getStoredState(offlinePersistConfig)])
  .then(([online, storedData]) => {
    ...
    const store = createStore(browserHistory, { client, restApp }, data, offlinePersistConfig);
    const history = syncHistoryWithStore(browserHistory, store);

    const renderRouter = props => <ReduxAsyncConnect
      {...props}
      helpers={{ client, restApp }}
    ...
    const render = routes => {
      match({ history, routes }, (error, redirectLocation, renderProps) => {
        ReactDOM.render(
          <HotEnabler>
            <Provider store={store} restApp={restApp} key="provider">
